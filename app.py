...
if use_basic_pitch:
    onset_threshold: float = st.sidebar.slider(
    ...
    frame_threshold: float = st.sidebar.slider(
...
max_fret: int = st.sidebar.select_slider(
    "Max Fret", options=[3, 5, 7, 12], value=7,
    help="Highest fret allowed.  Lower = easier; notes above are skipped.",
)
...
subdivision: int = st.sidebar.selectbox(
    "Note Grid", options=[1, 2, 4], index=1,
...
st.title("AutoTabber")
st.markdown(
    "Upload audio to generate a **beginner-friendly ASCII guitar tab**.  "
    "Basic-pitch works on full-mix songs; pYIN is best for isolated melodies."
)

col_up, col_yt = st.columns([3, 2])
with col_up:
    uploaded_file = st.file_uploader(
        "Upload Audio File", type=["mp3", "wav", "m4a"],
        help="MP3/M4A require ffmpeg.",
    )
with col_yt:
...
if uploaded_file is None:
    ...
    generate = st.button("Generate Tab", type="primary")
